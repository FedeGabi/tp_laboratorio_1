	...
	do {
		...
		case 2:
			if(!kilometraje && !flagAtencion)
			{
				system("cls");
				printf("\n");
				printf("              ***   ATENCION!, NO HA INGRESADO VALORES EN KILOMETROS   ***\n");
				printf("***   SI NO INGRESA NINGUN VALOR, LOS RESULTADOS RELACIONADOS CON ESTE DATO NO SE CALCULARAN  ***\n");
				...
				if(opcion==1)
				{
					...
					{
						system("cls");
						printf(" *** PRECIO AEROLINEAS: $ %.2f, SE HA CARGADO CON EXITO\n\n", precioAero);
						system("pause");
					}
				}
				else if(opcion==2)
				{
					...
					{
						system("cls");
						printf(" *** PRECIO LATAM: $ %.2f, SE HA CARGADO CON EXITO\n\n", precioLatam);
						system("pause");
					}
				}
			break;
		...
		case 4:
			...
			if (!flagCalculoAero && !flagCalculoLatam) {
				...
				if(flagCalculoAero){
					informarResultados(kilometraje, precioAero, resultadoDescuentoAero, resultadoInteresAero, resultadoBtcAero, precioUnitarioAero, "Aerolineas");
					printf("\n");
				}
				if(flagCalculoLatam){
					informarResultados(kilometraje, precioLatam, resultadoDescuentoLatam, resultadoInteresLatam, resultadoBtcLatam, precioUnitarioLatam, "Latam");
				}
		...
		case 5:
			system("cls");
			cargaForzada(&kilometraje, &precioAero, &precioLatam, 7090, 162965, 159339);
			hardcodearBanderas(&flagAtencion, &flagCalculoAero, &flagCalculoLatam, 1);
			calcularDescuento(precioAero, DESCUENTO, &resultadoDescuentoAero);
			calcularDescuento(precioLatam, DESCUENTO, &resultadoDescuentoLatam);
			calcularInteres(precioAero, INTERES, &resultadoInteresAero);
			calcularInteres(precioLatam, INTERES, &resultadoInteresLatam);
